...
def collect_image_paths(data_dir):
    ...
    crawler_ = futil.FileSystemDataCrawler(data_dir, image_keys, MyFilePathGenerator(), dir_filter)  # todo: modify here
...
def load_images(image_paths):
    ...
    image_dict = {
        structure.BrainImageTypes.T1w: sitk.ReadImage(image_paths[structure.BrainImageTypes.T1w], sitk.sitkFloat32),  # todo: modify here -> sitk.sitkFloat32 defines the pixle type
        structure.BrainImageTypes.GroundTruth: sitk.ReadImage(image_paths[structure.BrainImageTypes.GroundTruth], sitk.sitkUInt8)  # todo: modify here -< sitk.sitkUInt8 defines the pixle type
    }
...
def register_images(image_dict, atlas_img):
    ...
    registered_t1 = registration.execute(image_dict[structure.BrainImageTypes.T1w], registration_params)  # todo: modify here -> Represents human readable image types: T1 weighted = 1
